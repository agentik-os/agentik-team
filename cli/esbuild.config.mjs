/**
 * esbuild configuration for building the agentik-team CLI for npm.
 *
 * Bundles all workspace packages (@agentik-os/*) into a single file.
...
// to npm and resolved at runtime (e.g. @agentik-os/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@agentik-os/")) {
      externals.add(name);
    }
  }
