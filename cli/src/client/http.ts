...
export class PaperclipApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-agentik-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Agentik Team API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Agentik Team server is not running, the configured URL is wrong, or the request is being blocked before it reaches Paperclip.",
    "",
    "Try:",
    "- Start Agentik Team with `pnpm dev` or `pnpm agentik-team run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Paperclip is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`AGENTIK_API_URL\`.`,
  );
  return lines.join("\n");
}
