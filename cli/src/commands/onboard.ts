...
type OnboardDefaults = Pick<AgentikConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "AGENTIK_PUBLIC_URL",
  "DATABASE_URL",
  "AGENTIK_DB_BACKUP_ENABLED",
  "AGENTIK_DB_BACKUP_INTERVAL_MINUTES",
  "AGENTIK_DB_BACKUP_RETENTION_DAYS",
  "AGENTIK_DB_BACKUP_DIR",
  "AGENTIK_DEPLOYMENT_MODE",
  "AGENTIK_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "AGENTIK_ALLOWED_HOSTNAMES",
  "AGENTIK_AUTH_BASE_URL_MODE",
  "AGENTIK_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "AGENTIK_STORAGE_PROVIDER",
  "AGENTIK_STORAGE_LOCAL_DIR",
  "AGENTIK_STORAGE_S3_BUCKET",
  "AGENTIK_STORAGE_S3_REGION",
  "AGENTIK_STORAGE_S3_ENDPOINT",
  "AGENTIK_STORAGE_S3_PREFIX",
  "AGENTIK_STORAGE_S3_FORCE_PATH_STYLE",
  "AGENTIK_SECRETS_PROVIDER",
  "AGENTIK_SECRETS_STRICT_MODE",
  "AGENTIK_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const publicUrl =
    process.env.AGENTIK_PUBLIC_URL?.trim() ||
    process.env.AGENTIK_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.AGENTIK_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.AGENTIK_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.AGENTIK_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.AGENTIK_ALLOWED_HOSTNAMES
    ? process.env.AGENTIK_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.AGENTIK_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.AGENTIK_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.AGENTIK_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.AGENTIK_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.AGENTIK_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.AGENTIK_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.AGENTIK_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.AGENTIK_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.AGENTIK_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.AGENTIK_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.AGENTIK_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.AGENTIK_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.AGENTIK_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.AGENTIK_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.AGENTIK_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "AGENTIK_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<AgentikConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printAgentikCliBanner();
  p.intro(pc.bgCyan(pc.black(" agentik-team onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveAgentikInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: AgentikConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@agentik-os/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `agentik-team doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("AGENTIK_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.AGENTIK_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("AGENTIK_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("AGENTIK_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: AgentikConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: AGENTIK_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("agentik-team run")}`,
      `Reconfigure later: ${pc.cyan("agentik-team configure")}`,
      `Diagnose setup: ${pc.cyan("agentik-team doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (!shouldRunNow && !opts.invokedByRun && process.stdin.isTTY && process.stdout.isTTY) {
    const answer = await p.confirm({
      message: "Start Agentik Team now?",
      initialValue: true,
    });
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.AGENTIK_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("agentik-team run")}`,
        `Then: ${pc.cyan("agentik-team auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
