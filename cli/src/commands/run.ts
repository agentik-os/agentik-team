...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("agentik-team onboard")} once, then retry ${pc.cyan("agentik-team run")}.`);
      process.exit(1);
    }
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.AGENTIK_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@agentik-os/server/src/index.ts")) {
    process.env.AGENTIK_UI_DEV_MIDDLEWARE = "true";
  }
}
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @agentik-os/server package
  try {
    const mod = await import("@agentik-os/server");
    return await startServerFromModule(mod, "@agentik-os/server");
  } catch (err) {
