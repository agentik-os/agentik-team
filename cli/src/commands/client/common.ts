...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("-d, --data-dir <path>", "Agentik Team data directory root (isolates state from ~/.agentik-team)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Agentik Team API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.AGENTIK_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.AGENTIK_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.AGENTIK_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set AGENTIK_COMPANY_ID, or set context profile companyId via `agentik-team context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.AGENTIK_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.AGENTIK_SERVER_PORT || "");
