...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@agentik-os/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://agentik:agentik@127.0.0.1:${port}/agentik`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" agentik-team db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveAgentikInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "agentik-team";
