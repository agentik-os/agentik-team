...
const JWT_SECRET_ENV_KEY = "AGENTIK_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Agentik Team CLI commands",
...
export function resolveAgentikEnvFile(configPath?: string): string {
...
export function loadAgentikEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAgentikEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAgentikEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAgentikEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAgentikEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAgentikEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAgentikEnvEntries(next, filePath);
  return next;
}
