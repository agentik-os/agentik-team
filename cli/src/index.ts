...
const DATA_DIR_OPTION_HELP =
  "Agentik Team data directory root (isolates state from ~/.agentik-team)";

program
  .name("agentik-team")
  .description("Agentik Team CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadAgentikEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start Agentik Team immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Agentik Team setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "agentik-team")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Agentik Team")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Agentik Team server API")
