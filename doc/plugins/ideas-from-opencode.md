...
Agentik Team V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Agentik Team-shaped design.

Assumption for this document: Agentik Team is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Agentik Team objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Agentik Team.

The main conclusion is:

- Agentik Team should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Agentik Team should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Agentik Team should use multiple extension classes instead of one generic plugin bag:
...
If Agentik Team does this well, the examples you listed become straightforward:
...
Relevant Agentik Team files reviewed for current extension seams:
...
It is too dangerous for Agentik Team core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Agentik Team — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Agentik Team should absolutely do this.
...
Agentik Team should copy this discipline.
...
Agentik Team should adopt this style for plugin actions, automations, and UI schemas.
...
Agentik Team can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Agentik Team too.

## What Agentik Team Should Not Copy Directly
...
Agentik Team is a control plane for an operator-managed instance with company objects.
...
Agentik Team is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Agentik Team should not auto-load arbitrary code from a workspace repo like `.agentik-team/plugins` or project directories.
...
For Agentik Team, equivalent hooks into:
...
Agentik Team should not allow plugins to silently replace:
...
For Agentik Team it would be risky because it combines:
...
Agentik Team should require an explicit operator install step.

## Why Agentik Team Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Agentik Team |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Agentik Team instance |
...
That means Agentik Team should borrow the good ideas from `opencode` but use a stricter architecture.

## Agentik Team Already Has Useful Pre-Plugin Seams

Agentik Team has several extension-like seams already:
...
This is good news.
Agentik Team does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Agentik Team Plugin Model
...
Agentik Team already has this pattern implicitly:
...
- `plugin` means integration code that talks to Agentik Team through a typed plugin protocol and capability model
...
- invoke explicit Agentik Team actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@agentik-os/plugin-sdk/ui`) with design tokens and common components.
...
Agentik Team should treat plugin installation as a global instance-level action.

Examples:

- install `@agentik-team/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Agentik Team objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Agentik Team already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Agentik Team too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@agentik-team/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@agentik-os/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Agentik Team instance.

A `create-agentik-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Agentik Team achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Agentik Team server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Agentik Team's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@agentik-os/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@agentik-os/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Agentik Team

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@agentik-os/plugin-sdk";

export default definePlugin({
  id: "@agentik-team/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Agentik Team entities
    });
    ...
    ctx.events.on("plugin.@agentik-team/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@agentik-os/plugin-sdk/ui";
...
Any Agentik Team plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Agentik Team objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@agentik-team/plugin-linear`)
...
- reuse core tables when the data is clearly part of Agentik Team itself
...
If a concept is becoming part of Agentik Team's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Agentik Team itself
...
### 3. Scope plugin data to Agentik Team objects before adding custom schemas

A lot of plugin data naturally hangs off existing Agentik Team objects:
...
If Agentik Team eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@agentik-team/plugin-workspace-files`
...
Package idea: `@agentik-team/plugin-terminal`
...
Package idea: `@agentik-team/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Agentik Team.
- Board reviews the diff after a run without leaving Agentik Team.
...
- optional `events.emit` (e.g. `plugin.@agentik-team/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@agentik-team/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@agentik-team/plugin-linear`

This plugin syncs Agentik Team work with Linear. It is useful for:

- importing backlog from Linear
- linking Agentik Team issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Agentik Team and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Agentik Team.
- Agentik Team issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@agentik-team/plugin-github-issues`

This plugin syncs Agentik Team issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Agentik Team issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Agentik Team task governance
...
- Plugin settings:
  ...
  - whether new Agentik Team issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Agentik Team issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Agentik Team.
- GitHub webhooks update status/comment state in Agentik Team.
- A PR is linked back to the Agentik Team issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@agentik-team/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@agentik-team/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Agentik Team. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Agentik Team issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Agentik Team plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Agentik Team dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Agentik Team issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Agentik Team core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@agentik-team/plugin-runtime-processes`
...
Package idea: `@agentik-team/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Agentik Team. It is useful for:
...
- Failed payment spikes or churn events can generate Agentik Team issues for follow-up.
...
- Stripe data should stay additive to Agentik Team core
...
- plugin UI bundle loading, host bridge, `@agentik-os/plugin-sdk/ui`
...
- `@agentik-os/plugin-test-harness` and `create-agentik-plugin` starter template
...
Agentik Team should not implement "an OpenCode-style generic in-process hook system."
Agentik Team should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Agentik Team
...
8. Ship `@agentik-os/plugin-test-harness` and `create-agentik-plugin` starter template.
