# Agentik Team Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Agentik Team's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Agentik Team app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Agentik Team HTTP APIs directly.
...
Agentik Team plugin design is based on the following assumptions:

1. Agentik Team is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Agentik Team business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Agentik Team core.
...
2. Let plugins add major capabilities without editing Agentik Team core.
...
4. Depend on project-local plugin folders such as `.agentik-team/plugins`.
...
The single Agentik Team deployment an operator installs and controls.

### 5.2 Company

A first-class Agentik Team business object inside the instance.
...
A trusted in-process extension loaded directly by Agentik Team core.
...
An installable instance-wide extension package loaded through the Agentik Team plugin runtime.
...
Agentik Team has two extension classes.
...
Agentik Team already has a concrete workspace model:
...
Plugins live under the Agentik Team instance directory.

Suggested layout:

- `~/.agentik-team/instances/default/plugins/package.json`
- `~/.agentik-team/instances/default/plugins/node_modules/`
- `~/.agentik-team/instances/default/plugins/.cache/`
- `~/.agentik-team/instances/default/data/plugins/<plugin-id>/`
...
Agentik Team should add CLI commands:

- `pnpm agentik-team plugin list`
- `pnpm agentik-team plugin install <package[@version]>`
- `pnpm agentik-team plugin uninstall <plugin-id>`
- `pnpm agentik-team plugin upgrade <plugin-id> [version]`
- `pnpm agentik-team plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@agentik-team/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```json
{
  "name": "@agentik-team/plugin-linear",
  "version": "0.1.0",
  "agentik-teamPlugin": {
...
```ts
export interface Agentik TeamPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumAgentik TeamVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Agentik Team agents can use during runs.
...
- Agentik Team server starts one worker process per installed plugin
...
Receives one typed Agentik Team domain event.
...
export function definePlugin(definition: PluginDefinition): Agentik TeamPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: Agentik TeamPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@agentik-os/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@agentik-os/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@agentik-os/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@agentik-os/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@agentik-os/plugin-sdk/ui`
...
The `@agentik-os/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Agentik Team secret provider system rather than a plain text input
...
1. Core Agentik Team data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Agentik Team objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Agentik Team product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Agentik Team later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Agentik Team secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm agentik-team plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Agentik Team server. This is a normative requirement, not optional.
...
### 27.1 `@agentik-os/plugin-test-harness`
...
```ts
import { createTestHarness } from "@agentik-os/plugin-test-harness";
...
For developing a plugin against a running Agentik Team instance:

- The operator installs the plugin from a local path: `pnpm agentik-team plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-agentik-plugin`) that scaffolds:

- `package.json` with correct `agentik-teamPlugin` keys
...
- `@agentik-team/plugin-workspace-files`
- `@agentik-team/plugin-terminal`
- `@agentik-team/plugin-git`
- `@agentik-team/plugin-linear`
- `@agentik-team/plugin-github-issues`
- `@agentik-team/plugin-grafana`
- `@agentik-team/plugin-runtime-processes`
- `@agentik-team/plugin-stripe`
...
- `@agentik-os/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@agentik-os/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@agentik-os/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@agentik-os/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@agentik-os/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@agentik-os/plugin-sdk/ui`
...
- `@agentik-os/plugin-test-harness`
- `create-agentik-plugin` starter template
...
Agentik Team should not implement a generic in-process hook bag modeled directly after local coding tools.

Agentik Team should implement:
...
That is the complete target design for the Agentik Team plugin system.
