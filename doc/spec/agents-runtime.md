...
For `claude_local` and `codex_local`, Agentik Team assumes the CLI is already installed and authenticated on the host machine.
...
Agentik Team stores resumable session state per `(agent, taskKey, adapterType)`.
