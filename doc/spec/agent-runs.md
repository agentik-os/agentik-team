...
This spec defines how Agentik Team actually runs agents while staying runtime-agnostic.
...
1. Agentik Team is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Agentik Team manages lifecycle, and on exit Agentik Team parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `agentik-team.skill` (shared Agentik Team skill text block)
...
2. Preferred transport is env vars (`AGENTIK_*`) injected at runtime.
