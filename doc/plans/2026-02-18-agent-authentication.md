...
Agents need API keys to authenticate with Agentik Team. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Agentik Team
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Agentik Team generates a token and passes it directly to the agent
...
`AGENTIK_API_URL`, `AGENTIK_AGENT_ID`, `AGENTIK_COMPANY_ID`. We need to
add a `AGENTIK_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Agentik Team CLI
...
```
agentik-team auth login
# Opens browser -> user confirms -> token stored at ~/.agentik-team/credentials
```
...
managed by the Agentik Team server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Agentik Team.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Agentik Team SKILL.md (or a link to it)
   - What information Agentik Team needs from the agent (e.g. webhook URL, adapter
...
5. Agentik Team stores the pending registration.
...
7. On approval, Agentik Team provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Agentik Team issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.agentik-os.com/skills/agentik-team/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Agentik Team should send you heartbeats",
...
1. Generate a Agentik Team invite link for the company.
...
4. A Agentik Team company member approves the new agent.
5. Agentik Team begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `AGENTIK_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `agentik-team auth login` for developer-managed remote agents.                                      |
