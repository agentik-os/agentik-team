...
- New config/env settings:
  - `AGENTIK_AGENT_JWT_SECRET`
  - `AGENTIK_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `AGENTIK_AGENT_JWT_ISSUER` (default: `agentik-team`)
  - `AGENTIK_AGENT_JWT_AUDIENCE` (default: `agentik-team-api`)
...
1. In:
   ...
   inject `AGENTIK_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `AGENTIK_API_KEY`, do not overwrite it.
- Continue injecting:
  - `AGENTIK_AGENT_ID`
  - `AGENTIK_COMPANY_ID`
  - `AGENTIK_API_URL`
...
- Local adapters authenticate without manual `AGENTIK_API_KEY` config.
