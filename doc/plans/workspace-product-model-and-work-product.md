...
Agentik Team needs to support two very different but equally valid ways of working:
...
Today, Agentik Team already has the beginnings of this model:
...
- making Agentik Team feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Agentik Team users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Agentik Team deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Agentik Team into a full code review product
...
This object must be recorded explicitly so that Agentik Team can:
...
Agentik Team should treat PRs as a type of work product linked back to:
...
Agentik Team already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Agentik Team must be a first-class deployment mode

Agentik Team cannot assume that it is running on the same machine as the code.

In cloud deployments, Agentik Team may:
...
- `Runtime service`: a process or service Agentik Team owns or tracks for a workspace
...
It also lets cloud-hosted Agentik Team deployments point at codebases and remotes without pretending the Agentik Team host has direct filesystem access.
...
This lets Agentik Team support:
...
Without an explicit `execution workspace` record, Agentik Team has nowhere stable to attach:
...
Agentik Team needs a single place to show:
...
- Agentik Team-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Agentik Team owning the execution host
...
## 1. Agentik Team host and execution host must be decoupled

Agentik Team may run:
...
without the Agentik Team host itself running local git or local preview processes.
...
Agentik Team should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Agentik Team.
...
Agentik Team should keep workspace automation centered on a small set of hooks:
...
Agentik Team should provide the equivalent visibility in the project `Code` surface:
...
Agentik Team should treat execution workspaces as the natural home for:
...
Agentik Team should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Agentik Team should continue treating cleanup policy as part of the core workspace model:
...
Agentik Team should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Agentik Team host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Agentik Team should borrow Worktrunk's good execution patterns while keeping the broader Agentik Team model:
...
This is how Agentik Team supports:
...
That keeps Agentik Team focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Agentik Team deployments remain first-class

That is a better fit for Agentik Team than either extreme:
