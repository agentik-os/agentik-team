...
- local mode that still feels instant (`npx agentik-team run` and go)
...
2. CLI command `pnpm agentik-team auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm agentik-team onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm agentik-team onboard`).
...
- expose key instance settings in API + CLI (`agentik-team configure` / `agentik-team onboard`)
...
7. `pnpm agentik-team onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
