...
This document defines the next-stage plan for Agentik Team company import/export.

The core shift is:

- move from a Agentik Team-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Agentik Team.
...
Agentik Team already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Agentik Team
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `agentik-team.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.agentik-team.yaml` sidecar for high-fidelity Agentik Team-specific details
...
11. adapter-aware skill sync surfaces so Agentik Team can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Agentik Team at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Agentik Team sidecar.
...
Agentik Team must not redefine `SKILL.md`.
...
- Agentik Team-specific fidelity lives in `.agentik-team.yaml`
- Agentik Team may resolve and install `SKILL.md` packages, but it must not require a Agentik Team-only skill format
...
### 5.4 Base Package Vs Agentik Team Extension
...
- base package:
  ...
  - no Agentik Team-only runtime fields by default
- Agentik Team extension:
  - `.agentik-team.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Agentik Team tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`agentik-team.manifest.json` is not part of the future package direction.
...
Agentik Team import/export should support these entity kinds:
...
In Agentik Team V2 portability:
...
- Agentik Team can treat “this set of agents came from team package X” as the imported-team model
...
Agentik Team should support source references in package metadata with:
...
Agentik Team should define a new adapter surface area around skills:
...
Planned Agentik Team behavior:

- if an adapter supports read, Agentik Team should show current skills in the UI
- if an adapter supports write, Agentik Team should let the user enable/disable imported skills
- if an adapter supports sync, Agentik Team should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.agentik-team.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.agentik-team.yaml` alongside the base package
...
## 10. Storage Model Inside Agentik Team
...
Agentik Team should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `agentik-team company export <company-id> --out <path>`
- `agentik-team company import <path-or-url> --dry-run`
- `agentik-team company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Agentik Team storage, or only referenced at import time?
...
3. Should Agentik Team support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Agentik Team aligned with:
