...
This document defines the product and UI plan for skill management in Agentik Team.
...
- Agentik Team company import/export can include skills as package content
...
3. The UI must always tell the truth about what Agentik Team knows:
   - desired state in Agentik Team
...
Agentik Team should treat skills at two scopes:
...
- created locally inside Agentik Team later
...
7. “Understand what Agentik Team knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Agentik Team-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Agentik Team did not install.

These should be clearly marked:

- external
- not managed by Agentik Team
...
- `desired_only`: Agentik Team wants it, adapter does not show it yet
- `external`: adapter has it but Agentik Team does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Agentik Team only tracks desired state
...
- Agentik Team can still track desired skills
...
- Agentik Team can see adapter skills
...
- `.agentik-team.yaml` may add Agentik Team-specific fidelity, but should not replace the base shortname association model
...
- `.agentik-team.yaml` may add Agentik Team-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Agentik Team should have a company-scoped skill model or managed package model representing:
...
Agentik Team should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Agentik Team-specific.
...
That gives Agentik Team one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
