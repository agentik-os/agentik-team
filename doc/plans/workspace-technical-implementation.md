...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/agentik-team-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `AGENTIK_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `AGENTIK_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Agentik Team may create host-local worktrees, processes, and paths
- `cloud`
  - Agentik Team should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `agentik-team | github | vercel | s3 | custom`
...
- use `AGENTIK_EXECUTION_TOPOLOGY` as a defaulting guardrail
