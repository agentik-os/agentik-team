...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Agentik Team plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Agentik Team.
...
- package: `@agentik-os/plugin-kitchen-sink-example`
- plugin id: `agentik-team.kitchen-sink-example` or `agentik-team-kitchen-sink-example`

Recommendation: use `agentik-team-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Agentik Team event such as `issue.created`
...
Recommendation: default to a Agentik Team-local endpoint or a stable public echo endpoint to avoid flaky docs.
