...
This document defines the V1 design for a Agentik Team adapter that integrates with
...
4. synthesized stdout events for Agentik Team UI/CLI
...
3. **Agent callback auth to Agentik Team** uses a bootstrap exchange flow (no long-lived Agentik Team key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Agentik Team endpoints, not full SKILL.md prompt injection.
...
- `agentik-teamPublicUrl` (optional override; else `AGENTIK_PUBLIC_URL` env)
...
## Agentik Team Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `AGENTIK_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.agentik-teamPublicUrl`
2. `process.env.AGENTIK_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Agentik Team credentials in prompt text.

Flow:

1. Before launch/follow-up, Agentik Team mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
   - short TTL (for example 10 minutes)
2. Adapter includes only:
   - `agentik-teamPublicUrl`
   - exchange endpoint path
   - bootstrap token
3. Cursor agent calls:
   - `POST /api/agent-auth/exchange`
4. Agentik Team validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Agentik Team API calls.
...
1. Prompt includes a compact instruction to fetch skills from Agentik Team.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/agentik-team`
   - `GET /api/skills/agentik-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <agentik-teamPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Agentik Team run context.
...
5. `agentik-teamPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- agentik-team public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from Agentik Team skill endpoints |
| Agentik Team auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Agentik Team context.
3. Add richer PR metadata surfacing in Agentik Team UI.
