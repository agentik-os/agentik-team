...
`PAP-447` asks how Agentik Team should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Agentik Team instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Agentik Team:

- some operators use Agentik Team to manage Agentik Team and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Agentik Team
- Claude and Codex expose different built-in affordances, so Agentik Team should not overfit to one tool

## Core Product Decision

Agentik Team should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Agentik Team
...
- Agentik Team should not assume that every adapter can see or use a host filesystem path directly
...
If Agentik Team makes worktrees agent-first, it will blur:
...
- Agentik Team core resolves and records execution workspace state
...
That gives Agentik Team room to support:
...
- Agentik Team handles workspace runtime services under the hood using project/workspace policy
...
Once Agentik Team is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Agentik Team should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Agentik Team cannot infer
...
- when issue moves to done, should Agentik Team auto-commit?
- should it auto-push?
...
Agentik Team already has the right foundation for a project-first model.
...
For server-spawned local adapters, Agentik Team already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Agentik Team should mirror directly

Implication:

- for `codex_local`, Agentik Team should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Agentik Team should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Agentik Team should separate:
...
Agentik Team should be able to express intentions such as:
...
These are not specific to Agentik Team. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".agentik-team/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Agentik Team-specific `devServer` flag.
...
Agentik Team should distinguish between:
...
Agentik Team should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Agentik Team should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Agentik Team
...
For runtime services specifically, Agentik Team should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Agentik Team-only dev-server flags
...
3. Inject resolved execution workspace details into `context.agentik-teamWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Agentik Team instance can be reused through env/config injection when appropriate
...
- Agentik Team can decide whether to reuse or start a fresh service deterministically
...
3. Test against Agentik Team-on-Agentik Team workflows first.
...
7. Workspace runtime services are modeled generically, not as Agentik Team-specific dev-server toggles.
