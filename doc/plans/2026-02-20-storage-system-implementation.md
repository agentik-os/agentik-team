...
Add a single storage subsystem for Agentik Team that supports:
...
- API serves attachment bytes through authenticated Agentik Team endpoints.
...
- Default local path is under instance root: `~/.agentik-team/instances/<instanceId>/data/storage`.
...
- `agentik-team onboard` writes a valid `storage` config block by default.
- `agentik-team configure --section storage` can switch between local and s3 modes.
...
- `agentik-team doctor` reports actionable storage status.
