...
ClipHub is the public registry where people share, discover, and download Agentik Team company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Agentik Team what a package registry is to a programming language. Agentik Team already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Agentik Team — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Agentik Team instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Agentik Team spec. It contains:
...
**Install (fresh start):**
```
agentik-team install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Agentik Team instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Agentik Team, export your company as a template, then publish:

```
agentik-team export --template my-company
agentik-team publish cliphub my-company
```
...
ClipHub is a **separate service** from Agentik Team itself. Agentik Team is self-hosted; ClipHub is a hosted registry that Agentik Team instances talk to.
...
| **Agentik Team CLI** | `agentik-team install`, `agentik-team publish`, `agentik-team cliphub sync` — built into Agentik Team |
| **Agentik Team UI** | "Browse ClipHub" panel in the Agentik Team web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Agentik Team) |
| Backend | TypeScript + Hono (consistent with Agentik Team) |
| Database | PostgreSQL |
...
4. Run `agentik-team install cliphub:acme/lean-saas-shop`
5. Agentik Team creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Agentik Team until it works well
2. Export: `agentik-team export --template my-agency`
3. Publish: `agentik-team publish cliphub my-agency`
...
3. Install just that agent: `agentik-team install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Agentik Team

ClipHub is **not required** to use Agentik Team. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Agentik Team what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `agentik-team install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Agentik Team web UI
- [ ] `agentik-team cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Agentik Team's own philosophy)
