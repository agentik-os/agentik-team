...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://agentik:agentik@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "agentik");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://agentik:agentik@127.0.0.1:${preferredPort}/agentik`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://agentik:agentik@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "agentik");
    return {
      connectionString: `postgres://agentik:agentik@127.0.0.1:${port}/agentik`,
  ...
  const adminConnectionString = `postgres://agentik:agentik@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "agentik");

  return {
    connectionString: `postgres://agentik:agentik@127.0.0.1:${selectedPort}/agentik`,
