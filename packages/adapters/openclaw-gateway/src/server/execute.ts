...
import { asNumber, asString, buildPaperclipEnv, parseObject } from "@agentik-os/adapter-utils/server-utils";
...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const orderedKeys = [
    ...
    "AGENTIK_TASK_ID",
    "AGENTIK_WAKE_REASON",
    "AGENTIK_WAKE_COMMENT_ID",
    "AGENTIK_APPROVAL_ID",
    "AGENTIK_APPROVAL_STATUS",
    "AGENTIK_LINKED_ISSUE_IDS",
  ];
  ...
  const lines = [
    ...
    ...envLines,
    `AGENTIK_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load AGENTIK_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $AGENTIK_API_KEY on every API call.",
    "- Use X-Paperclip-Run-Id: $AGENTIK_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: AGENTIK_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$AGENTIK_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$AGENTIK_COMPANY_ID/issues?assigneeAgentId=$AGENTIK_AGENT_ID&status=todo,in_progress,blocked",
