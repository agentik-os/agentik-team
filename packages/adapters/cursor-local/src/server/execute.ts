...
export async function ensureCursorSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[agentik-team] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[agentik-team] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[agentik-team] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[agentik-team] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  if (wakeTaskId) {
    env.AGENTIK_TASK_ID = wakeTaskId;
  }
  if (wakeReason) {
    env.AGENTIK_WAKE_REASON = wakeReason;
  }
  if (wakeCommentId) {
    env.AGENTIK_WAKE_COMMENT_ID = wakeCommentId;
  }
  if (approvalId) {
    env.AGENTIK_APPROVAL_ID = approvalId;
  }
  if (approvalStatus) {
    env.AGENTIK_APPROVAL_STATUS = approvalStatus;
  }
  if (linkedIssueIds.length > 0) {
    env.AGENTIK_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  }
  if (effectiveWorkspaceCwd) {
    env.AGENTIK_WORKSPACE_CWD = effectiveWorkspaceCwd;
  }
  if (workspaceSource) {
    env.AGENTIK_WORKSPACE_SOURCE = workspaceSource;
  }
  if (workspaceId) {
    env.AGENTIK_WORKSPACE_ID = workspaceId;
  }
  if (workspaceRepoUrl) {
    env.AGENTIK_WORKSPACE_REPO_URL = workspaceRepoUrl;
  }
  if (workspaceRepoRef) {
    env.AGENTIK_WORKSPACE_REPO_REF = workspaceRepoRef;
  }
  if (agentHome) {
    env.AGENT_HOME = agentHome;
  }
  if (workspaceHints.length > 0) {
    env.AGENTIK_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  }
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[agentik-team] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: agentikEnvNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[agentik-team] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
