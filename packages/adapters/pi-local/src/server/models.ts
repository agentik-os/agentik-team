...
import type { AdapterModel } from "@agentik-os/adapter-utils";
import { asString, runChildProcess } from "@agentik-os/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AGENTIK_PI_COMMAND === "string" &&
    process.env.AGENTIK_PI_COMMAND.trim().length > 0
      ? process.env.AGENTIK_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["AGENTIK_", "npm_", "NPM_"] as const;
