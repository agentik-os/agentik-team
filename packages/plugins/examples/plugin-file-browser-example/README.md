...
```bash
pnpm --filter @agentik-os/plugin-file-browser-example build
pnpm agentik-team plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm agentik-team plugin uninstall paperclip-file-browser-example --force
```
...
- Optional: use `agentik-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
