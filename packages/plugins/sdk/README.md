# `@agentik-os/plugin-sdk`

Official TypeScript SDK for Paperclip plugin authors.

- **Worker SDK:** `@agentik-os/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@agentik-os/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@agentik-os/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@agentik-os/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@agentik-os/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@agentik-os/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@agentik-os/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@agentik-os/plugin-sdk/ui/hooks` | Hooks only |
| `@agentik-os/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@agentik-os/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@agentik-os/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@agentik-os/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@agentik-os/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@agentik-os/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @agentik-os/plugin-sdk
```
...
```ts
import { definePlugin, runWorker } from "@agentik-os/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@agentik-os/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@agentik-os/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@agentik-os/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@agentik-os/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@agentik-os/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@agentik-os/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@agentik-os/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@agentik-os/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@agentik-os/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@agentik-os/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@agentik-os/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@agentik-os/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@agentik-os/plugin-sdk/bundlers";
...
```bash
agentik-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@agentik-os/plugin-sdk/dev-server";
