...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".agentik-sdk");
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "agentik-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["agentik", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@agentik-os/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@agentik-os/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@agentik-os/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@agentik-os/plugin-sdk\` and \`@agentik-os/shared\` from a local Agentik Team checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.agentik-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@agentik-os/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-agentik-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <agentik-sdk-path>]");
    process.exit(1);
  }
