 ...
 *
 * Plugin bundles are built with `external: ["@agentik-os/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__agentikPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__agentikPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__agentikPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__agentikPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@agentik-os/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@agentik-os/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@agentik-os/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@agentik-os/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@agentik-os/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__agentikPluginBridge__) {
 ...
 * self-register via `window.agentikPlugins.registerReactComponent()`.
 ...
 * of `@agentik-os/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
