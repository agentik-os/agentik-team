...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Agentik Team API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "agentikApiUrl",
                  String(config.agentikApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "agentikApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://agentik-team.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "agentik-team"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="agentik-team"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Agentik Team persists a device key during onboarding so pairing approvals
