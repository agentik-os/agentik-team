...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("agentik-worktree-enabled") !== "true") return null;

  const name = readMetaContent("agentik-worktree-name");
  const color = normalizeHexColor(readMetaContent("agentik-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("agentik-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
