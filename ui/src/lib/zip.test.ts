...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("agentik-team-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("agentik-team-demo/COMPANY.md");
  ...
  it("reads an Agentik Team zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".agentik-team.yaml": "schema: agentik-team/v1\n",
      },
      "agentik-team-demo",
    );

    await expect(readZipArchive(archive)).resolves.toEqual({
      rootPath: "agentik-team-demo",
      files: {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".agentik-team.yaml": "schema: agentik-team/v1\n",
  ...
  it("reads standard DEFLATE zip archives created outside Agentik Team", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("agentik-team-demo");
