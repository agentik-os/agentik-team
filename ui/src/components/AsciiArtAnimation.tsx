...
type AgentikSprite = (typeof AGENTIK_SPRITES)[number];
...
function spriteSize(sprite: AgentikSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = AGENTIK_SPRITES[(baseCol + baseRow) % AGENTIK_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = AGENTIK_SPRITES[Math.floor(Math.random() * AGENTIK_SPRITES.length)]!;
