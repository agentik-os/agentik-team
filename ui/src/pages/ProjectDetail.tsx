...
import { PROJECT_COLORS, isUuidLike, type BudgetPolicySummary } from "@agentik-os/shared";
...
export function ProjectDetail() {
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`agentik:project-tab:${project.id}`); } catch {}
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`agentik:project-tab:${project.id}`, tab); } catch {}
    }
