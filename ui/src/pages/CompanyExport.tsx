...
/**
 * Filter .agentik-team.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterAgentikYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://agentik-os.com) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm agentik-team company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Agentik Team](https://agentik-os.com) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Agentik Team](https://agentik-os.com) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .agentik-team.yaml and README.md content whenever checked files
