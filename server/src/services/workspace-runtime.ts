...
import type { AdapterRuntimeServiceReport } from "@agentik-os/adapter-utils";
import type { Db } from "@agentik-os/db";
import { workspaceRuntimeServices } from "@agentik-os/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.AGENTIK_WORKSPACE_CWD = input.worktreePath;
  env.AGENTIK_WORKSPACE_PATH = input.worktreePath;
  env.AGENTIK_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.AGENTIK_WORKSPACE_BRANCH = input.branchName;
  env.AGENTIK_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.AGENTIK_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.AGENTIK_WORKSPACE_SOURCE = input.base.source;
  env.AGENTIK_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.AGENTIK_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.AGENTIK_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.AGENTIK_PROJECT_ID = input.base.projectId ?? "";
  env.AGENTIK_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.AGENTIK_AGENT_ID = input.agent.id;
  env.AGENTIK_AGENT_NAME = input.agent.name;
  env.AGENTIK_COMPANY_ID = input.agent.companyId;
  env.AGENTIK_ISSUE_ID = input.issue?.id ?? "";
  env.AGENTIK_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.AGENTIK_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.AGENTIK_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.AGENTIK_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.AGENTIK_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.AGENTIK_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.AGENTIK_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.AGENTIK_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.AGENTIK_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.AGENTIK_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.AGENTIK_PROJECT_ID = input.workspace.projectId ?? "";
  env.AGENTIK_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.AGENTIK_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".agentik-team", "worktrees");
