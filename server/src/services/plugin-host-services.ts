...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@agentik-os/db";
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@agentik-os/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
