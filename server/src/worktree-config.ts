...
import type { AgentikConfig } from "@agentik-os/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Agentik Team worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.AGENTIK_IN_WORKTREE !== "true") return null;

  const configPath = resolveAgentikConfigPath(overrideConfigPath);
  const envPath = resolveAgentikEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.AGENTIK_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.AGENTIK_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.AGENTIK_HOME) ??
      nonEmpty(env.AGENTIK_WORKTREES_DIR) ??
      "~/.agentik-team-worktrees",
  );
...
function writeConfigFile(configPath: string, config: AgentikConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".agentik-team", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as AgentikConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: AgentikConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: AgentikConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.AGENTIK_HOME = context.homeDir;
  process.env.AGENTIK_INSTANCE_ID = context.instanceId;
  process.env.AGENTIK_CONFIG = context.configPath;
  process.env.AGENTIK_CONTEXT = context.contextPath;
  process.env.AGENTIK_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgentikConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    AGENTIK_HOME: context.homeDir,
    AGENTIK_INSTANCE_ID: context.instanceId,
    AGENTIK_CONFIG: context.configPath,
    AGENTIK_CONTEXT: context.contextPath,
    AGENTIK_IN_WORKTREE: "true",
    AGENTIK_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: AgentikConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgentikConfig;
  } catch {
    return;
  }
