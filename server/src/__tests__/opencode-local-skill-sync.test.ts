...
describe("opencode local skill sync", () => {
  ...
  it("reports configured Paperclip skills and installs them into the shared Claude/OpenCode skills home", async () => {
    const home = await makeTempDir("agentik-opencode-skill-sync-");
    ...
    const after = await syncOpenCodeSkills(ctx, [agentikKey]);
    expect(after.entries.find((entry) => entry.key === agentikKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".claude", "skills", "agentik"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled Paperclip skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("agentik-opencode-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncOpenCodeSkills(configuredCtx, [agentikKey]);
