...
describe("agent local JWT", () => {
  const secretEnv = "AGENTIK_AGENT_JWT_SECRET";
  const ttlEnv = "AGENTIK_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "AGENTIK_AGENT_JWT_ISSUER";
  const audienceEnv = "AGENTIK_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "agentik-team";
    process.env[audienceEnv] = "agentik-team-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
