...
import { ensureCursorSkillsInjected } from "@agentik-os/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Paperclip skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("agentik-cursor-skills-src-");
    const skillsHome = await makeTempDir("agentik-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "agentik");
    await createSkillDir(skillsDir, "agentik-create-agent");
    ...
    const injectedA = path.join(skillsHome, "agentik");
    const injectedB = path.join(skillsHome, "agentik-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "agentik")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "agentik-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "agentik-team"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "agentik-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("agentik-cursor-preserve-src-");
    const skillsHome = await makeTempDir("agentik-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "agentik");
    ...
    expect((await fs.lstat(path.join(skillsHome, "agentik-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("agentik-cursor-fail-src-");
    const skillsHome = await makeTempDir("agentik-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
