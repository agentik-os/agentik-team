...
import type { CompanyPortabilityFileEntry } from "@agentik-os/shared";
...
describe("company portability", () => {
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/agentik-team/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "agentik-team",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/agentik-team/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "agentik-team",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Paperclip extension data", async () => {
    ...
    expect(asTextFile(exported.files["skills/agentik-os/agentik-team/paperclip/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/agentik-os/agentik-team/paperclip/references/api.md"]).toBeUndefined();
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/agentik-os/agentik-team/paperclip/SKILL.md"]).toBeUndefined();
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        ...
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/agentik-os/agentik-team/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/agentik-os/agentik-team/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/agentik-os/agentik-team/release-changelog/SKILL.md"])).toContain("agentik-os/agentik-team/release-changelog");
  });
