...
import type { agents } from "@agentik-os/db";
import { sessionCodec as codexSessionCodec } from "@agentik-os/adapter-codex-local/server";
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[agentik-team] Using fallback workspace\n",
    });
  });
});
