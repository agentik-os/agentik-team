...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "agentik-team", LOGNAME: "agentik-team", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "agentik-team" }),
      },
    );

    expect(tokens).toEqual(["agentik-team", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "agentik-team" }, {
        userInfo: () => ({ username: "agentik-team" }),
      });

      expect(tokens).toEqual(["agentik-team", "custom-token"]);
    } finally {
  ...
  it("reports matches without leaking which token was searched", () => {
    ...
    const exitCode = runForbiddenTokenCheck({
      repoRoot: "/repo",
      tokens: ["agentik-team", "custom-token"],
      exec,
      log,
      error,
    });
