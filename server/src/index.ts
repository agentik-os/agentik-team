...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.AGENTIK_SECRETS_PROVIDER === undefined) {
    process.env.AGENTIK_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.AGENTIK_SECRETS_STRICT_MODE === undefined) {
    process.env.AGENTIK_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.AGENTIK_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.AGENTIK_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.AGENTIK_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.AGENTIK_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set AGENTIK_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.AGENTIK_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  if (config.deploymentMode === "authenticated") {
    const useClerk = Boolean(process.env.CLERK_SECRET_KEY?.trim());

    if (useClerk) {
      // Clerk-based authentication
      const { clerk, verifyClerkSession: verifyClerk } = await import("./auth/clerk.js");
      if (!clerk) {
        throw new Error("authenticated mode with Clerk requires CLERK_SECRET_KEY to be set");
      }
      logger.info("Using Clerk for authentication");
      // No betterAuthHandler needed — Clerk handles auth client-side
      // resolveSession uses Clerk's __session cookie
      resolveSession = async (req) => {
        ...
        const result = await verifyClerk(sessionToken);
        if (!result) return null;
        return {
          session: { id: `clerk:${result.userId}`, userId: result.userId },
      ...
      // Legacy better-auth authentication
      const {
      ...
      const betterAuthSecret =
        process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.AGENTIK_AGENT_JWT_SECRET?.trim();
      if (!betterAuthSecret) {
        throw new Error(
          "authenticated mode requires BETTER_AUTH_SECRET (or AGENTIK_AGENT_JWT_SECRET) to be set",
        );
      }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.AGENTIK_LISTEN_HOST = runtimeListenHost;
  process.env.AGENTIK_LISTEN_PORT = String(listenPort);
  process.env.AGENTIK_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.AGENTIK_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.AGENTIK_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
