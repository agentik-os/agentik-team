...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@agentik-os/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  agentikUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).agentikUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.agentikUpgradeContext = context;
