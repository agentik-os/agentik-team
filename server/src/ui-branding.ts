...
const RUNTIME_BRANDING_BLOCK_START = "<!-- AGENTIK_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- AGENTIK_RUNTIME_BRANDING_END -->";
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.AGENTIK_WORKTREE_NAME) ?? nonEmpty(env.AGENTIK_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.AGENTIK_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="agentik-worktree-enabled" content="true" />',
    `<meta name="agentik-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="agentik-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="agentik-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
