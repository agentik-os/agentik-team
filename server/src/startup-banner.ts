...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.AGENTIK_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.AGENTIK_AGENT_JWT_SECRET === "string" ? parsed.AGENTIK_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm agentik-team onboard`)",
  };
}
