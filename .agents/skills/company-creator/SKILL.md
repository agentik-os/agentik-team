...
description: >
  ...
  is already running in Agentik Team.
---
...
└── .agentik-team.yaml            (Agentik Team vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .agentik-team.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [Agentik Team](https://github.com/agentik-os/agentik-team)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to Agentik Team (https://github.com/agentik-os/agentik-team)
- A "Getting Started" section explaining how to import: `agentik-team company import --from <path>`
...
## .agentik-team.yaml Guidelines

The `.agentik-team.yaml` file is the Agentik Team vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — Agentik Team will use its default. Specifying an unknown adapter type causes an import error.

Agentik Team's supported adapter types (these are the ONLY valid values):
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.agentik-team.yaml`.
