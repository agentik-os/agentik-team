---
name: agentik-create-plugin
description: >
  Create new Agentik Team plugins with the current alpha SDK/runtime. Use when
...
# Create a Agentik Team Plugin

Use this skill when the task is to create, scaffold, or document a Agentik Team plugin.
...
node packages/plugins/create-agentik-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Agentik Team repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.agentik-team-sdk/`:

```bash
pnpm --filter @agentik-os/create-agentik-plugin build
node packages/plugins/create-agentik-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Agentik Team from an absolute local path during development
