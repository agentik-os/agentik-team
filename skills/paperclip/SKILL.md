...
Manual local CLI mode (outside heartbeat runs): use `agentik-team agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install Paperclip skills for Claude/Codex and print/export the required `AGENTIK_*` environment variables for that agent identity.

**Run audit trail:** You MUST include `-H 'X-Paperclip-Run-Id: $AGENTIK_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.
...
Headers: Authorization: Bearer $AGENTIK_API_KEY, X-Paperclip-Run-Id: $AGENTIK_RUN_ID
...
Headers: X-Paperclip-Run-Id: $AGENTIK_RUN_ID
