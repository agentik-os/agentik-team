...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Agentik Team-Signature` + `X-Agentik Team-Timestamp` header pair matching the trigger's signing mode.
