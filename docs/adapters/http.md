...
The `http` adapter sends a webhook request to an external agent service. The agent runs externally and Agentik Team just triggers it.
...
1. Agentik Team sends a POST request to the configured URL
2. The request body includes the execution context (agent ID, task info, wake reason)
3. The external agent processes the request and calls back to the Agentik Team API
...
The external agent uses `AGENTIK_API_URL` and an API key to call back to Agentik Team.
