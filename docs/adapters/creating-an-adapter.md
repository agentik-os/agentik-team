...
Build a custom adapter to connect Agentik Team to any agent runtime.
...
2. Build environment with `buildAgentik TeamEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `agentik-team run --watch` using `picocolors`.
...
Make Agentik Team skills discoverable to your agent runtime without writing to the agent's working directory:
