...
summary: Run Agentik Team with Tailscale-friendly host binding and connect from other devices
---

Use this when you want to access Agentik Team over Tailscale (or a private LAN/VPN) instead of only `localhost`.

## 1. Start Agentik Team in private authenticated mode
...
From the machine running Agentik Team:
...
## 3. Open Agentik Team from another device

Use the Tailscale IP or MagicDNS host with the Agentik Team port:
...
If you access Agentik Team with a custom private hostname, add it to the allowlist:
