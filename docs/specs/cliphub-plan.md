# ClipHub: Marketplace for Agentik Team Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Agentik Team configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Agentik Team-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Agentik Team company (agents, projects, governance) |
...
A complete Agentik Team company configuration:
...
Single-agent configurations designed to plug into a Agentik Team org:
...
Portable skill files that any Agentik Team agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Agentik Team's skill loading system
...
interface Listing {
  ...
  agentik-teamVersionMin: string;     // Minimum Agentik Team version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Agentik Team company
...
| `POST` | `/api/listings/:id/install` | Install to Agentik Team company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Agentik Team company (or create new)
  → ClipHub API calls Agentik Team API to:
  ...
  → Redirect to Agentik Team dashboard with new team running
```
...
### 5.3 Creator: Export from Agentik Team → Publish

```
Running Agentik Team company → "Export as Blueprint" (CLI or UI)
  → Agentik Team exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Agentik Team brand blue/purple)
...
| Install flow | One-click deploy to Agentik Team company |
...
{
  "targetCompanyId": "uuid",         // Existing Agentik Team company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `agentik-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Agentik Team
- **Backend**: Node.js API (or extend Agentik Team server)
- **Database**: Postgres (can share Agentik Team's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Agentik Team auth (or OAuth2)

### 9.2 Integration with Agentik Team

ClipHub can be:
- **Option A**: A separate app that calls Agentik Team's API to install blueprints
- **Option B**: A built-in section of the Agentik Team UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Agentik Team UI and API.
...
- [ ] Install flow: blueprint → Agentik Team company
...
- [ ] "Export from Agentik Team" CLI command
