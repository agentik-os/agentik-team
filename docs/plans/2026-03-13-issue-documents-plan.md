...
Add first-class **documents** to Agentik Team as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Agentik Team workflow/docs.
...
Update the Agentik Team agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/agentik-team/SKILL.md`
