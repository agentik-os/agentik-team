...
```sh
agentik-team company export <company-id> --out ./my-export
```
...
agentik-team company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
agentik-team company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
agentik-team company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
```sh
# From a local folder
agentik-team company import ./my-export

# From a GitHub URL
agentik-team company import https://github.com/org/repo

# From a GitHub subfolder
agentik-team company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
agentik-team company import org/repo
agentik-team company import org/repo/companies/acme
```
...
```sh
agentik-team company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
agentik-team company import org/company-templates/engineering-team \
  --target new \
...
```sh
agentik-team company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
agentik-team company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
agentik-team company import ./package \
  --target new \
  --yes \
  --json
```
