...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Agentik Team.
...
- be useful outside Agentik Team
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.agentik-team.yaml`
...
Example Agentik Team extension:
...
- Agentik Team must not require extra top-level fields for skill validity
- Agentik Team-specific extensions must live under `metadata.agentik-team` or `metadata.sources`
...
For Agentik Team, the preferred fidelity extension is:
...
- issue/task Agentik Team-only metadata
...
- Agentik Team tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Agentik Team shape:

```yaml
schema: agentik-team/v1
agents:
  claudecoder:
    adapter:
      type: claude_local
...
Additional rules for Agentik Team exporters:
...
- omit empty and default-valued Agentik Team fields when possible
...
- Agentik Team exporters should emit `.agentik-team.yaml` as a sidecar by default
...
## 19. Agentik Team Mapping

Agentik Team can map this spec to its runtime model like this:
...
- Agentik Team extension:
  - `.agentik-team.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Agentik Team-specific fidelity

Inline Agentik Team-only metadata that must live inside a shared markdown file should use:

- `metadata.agentik-team`

That keeps the base format broader than Agentik Team.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Agentik Team.

## 20. Cutover

Agentik Team should cut over to this markdown-first package model as the primary portability format.

`agentik-team.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Agentik Team, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
