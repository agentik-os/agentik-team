...
Environment overrides:
  ...
  AGENTIK_HOST=127.0.0.1    Host passed to the onboarded server
  AGENTIK_PORT=3232         Port passed to the onboarded server
...
export AGENTIK_HOST="${AGENTIK_HOST:-127.0.0.1}"
export AGENTIK_PORT="${AGENTIK_PORT:-3100}"
export AGENTIK_OPEN_ON_LISTEN="${AGENTIK_OPEN_ON_LISTEN:-false}"
...
echo "AGENTIK_HOST: $AGENTIK_HOST"
echo "AGENTIK_PORT: $AGENTIK_PORT"
...
env \
  ...
  HOST="$AGENTIK_HOST" \
  PORT="$AGENTIK_PORT" \
  AGENTIK_OPEN_ON_LISTEN="$AGENTIK_OPEN_ON_LISTEN" \
  pnpm agentik-team onboard --yes --data-dir "$PC_DATA"
