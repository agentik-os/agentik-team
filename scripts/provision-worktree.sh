...
base_cwd="${AGENTIK_WORKSPACE_BASE_CWD:?AGENTIK_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${AGENTIK_WORKSPACE_CWD:?AGENTIK_WORKSPACE_CWD is required}"
paperclip_home="${AGENTIK_HOME:-$HOME/.paperclip}"
paperclip_instance_id="${AGENTIK_INSTANCE_ID:-default}"
paperclip_dir="$worktree_cwd/.paperclip"
worktree_config_path="$paperclip_dir/config.json"
worktree_env_path="$paperclip_dir/.env"
worktree_name="${AGENTIK_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${AGENTIK_CONFIG:-}"
...
run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm agentik-team --help >/dev/null 2>&1; then
    pnpm agentik-team worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v agentik-team >/dev/null 2>&1; then
    agentik-team worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  AGENTIK_DIR="$paperclip_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  AGENTIK_WORKTREES_DIR="${AGENTIK_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const paperclipDir = process.env.AGENTIK_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.AGENTIK_WORKTREES_DIR) ?? "~/.paperclip-worktrees"));
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.AGENTIK_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.AGENTIK_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.AGENTIK_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "AGENTIK_HOME=" + JSON.stringify(worktreeHome),
    "AGENTIK_INSTANCE_ID=" + JSON.stringify(instanceId),
    "AGENTIK_CONFIG=" + JSON.stringify(configPath),
    "AGENTIK_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "AGENTIK_IN_WORKTREE=true",
    "AGENTIK_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.AGENTIK_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("AGENTIK_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "agentik-team CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
