...
AGENTIK_DEPLOYMENT_MODE="${AGENTIK_DEPLOYMENT_MODE:-authenticated}"
AGENTIK_DEPLOYMENT_EXPOSURE="${AGENTIK_DEPLOYMENT_EXPOSURE:-private}"
AGENTIK_PUBLIC_URL="${AGENTIK_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e AGENTIK_DEPLOYMENT_MODE="$AGENTIK_DEPLOYMENT_MODE" \
      -e AGENTIK_DEPLOYMENT_EXPOSURE="$AGENTIK_DEPLOYMENT_EXPOSURE" \
      -e AGENTIK_PUBLIC_URL="$AGENTIK_PUBLIC_URL" \
      -e AGENTIK_HOME="/paperclip" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "agentik-team@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$AGENTIK_HOME" --base-url "$AGENTIK_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$AGENTIK_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$AGENTIK_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$AGENTIK_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$AGENTIK_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$AGENTIK_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$AGENTIK_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $AGENTIK_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $AGENTIK_DEPLOYMENT_MODE/$AGENTIK_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$AGENTIK_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $AGENTIK_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$AGENTIK_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $AGENTIK_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
